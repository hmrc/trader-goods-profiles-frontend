...
class TraderProfileSpec extends AnyFreeSpec with Matchers with TryValues with OptionValues {
  ...
  ".validateNirmsNumber" - {
    ...
    "must validate Nirms" - {
      ...
      "user has answered HasNirmsUpdate as true and answered NirmsNumberUpdate" in {
        val answers =
          UserAnswers(userAnswersId)
            .set(HasNirmsUpdatePage, true)
            .success
            .value
            .set(NirmsNumberUpdatePage, "123456")
            .success
            .value
  ...
  "must return errors" - {
    ...
    "when user has answered HasNirmsUpdate as false and answered NirmsNumberUpdate" in {
      val answers =
        UserAnswers(userAnswersId)
          .set(HasNirmsUpdatePage, false)
          .success
    ...
    "when user has answered HasNirmsUpdate as true and not NirmsNumberUpdate" in {
      val answers =
        UserAnswers(userAnswersId)
          .set(HasNirmsUpdatePage, true)
          .success
      ...
      inside(result) { case Left(errors) =>
        errors.toChain.toList must contain theSameElementsAs Seq(
          PageMissing(NirmsNumberUpdatePage)
        )
      }
    }

    "when user has not answered HasNirmsUpdate or NirmsNumberUpdate" in {
      val answers = UserAnswers(userAnswersId)
