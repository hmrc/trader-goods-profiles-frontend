...
import models.{AdviceStatus, DeclarableStatus, GoodsRecordsPagination, ReviewReason}
import play.api.libs.json.*

import java.time.Instant

class GetRecordsResponseSpec extends SpecBase {
  ...
  private val getRecordsResponse: GetRecordsResponse = GetRecordsResponse(
    goodsItemRecords = Seq(getGoodsRecordResponse),
    pagination = GoodsRecordsPagination(1, 1, 1, None, None)
  )

  private val getRecordsResponseJson = Json.obj(
    "goodsItemRecords" -> Json.arr(getGoodsRecordResponseJson),
    "pagination"       -> Json.obj(
      "totalRecords" -> 1,
      "currentPage"  -> 1,
      "totalPages"   -> 1
  ...
  "GetRecordsResponse" - {
    "must deserialize from json" in {
      Json.fromJson[GetRecordsResponse](getRecordsResponseJson) mustBe JsSuccess(getRecordsResponse)
    }

    "must serialize to json" in {
      Json.toJson(getRecordsResponse) mustBe getRecordsResponseJson
    }
  }
