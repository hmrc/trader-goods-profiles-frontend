...
class UkimsNumberControllerSpec extends SpecBase with MockitoSugar {
  ...
  "UkimsNumberController" - {
    ...
    ".create" - {
      ...
      val ukimsNumberRoute = controllers.profile.routes.UkimsNumberController.onPageLoadCreate(NormalMode).url
      ...
      "must return OK and the correct view for a GET" in {
        ...
        running(application) {
          ...
          contentAsString(result) mustEqual view(
            form,
            controllers.profile.routes.UkimsNumberController.onSubmitCreate(NormalMode),
    ...
    ".update" - {
      ...
      "must return OK and the correct view for a GET with all trader profile complete" in {
        ...
        running(application) {
          ...
          contentAsString(result) mustEqual view(
            form.fill("1"),
            controllers.profile.routes.UkimsNumberController.onSubmitUpdate(NormalMode)
          )(
            request,
            messages(application)
          ).toString
      ...
      "must return OK and the correct view for a GET with all trader profile complete if the user is returning from the CYA page and UKIMS number should be filled in" in {
        ...
        running(application) {
          ...
          contentAsString(result) mustEqual view(
            form.fill(newUkims),
            controllers.profile.routes.UkimsNumberController.onSubmitUpdate(CheckMode)
          )(
            request,
            messages(application)
          ).toString
