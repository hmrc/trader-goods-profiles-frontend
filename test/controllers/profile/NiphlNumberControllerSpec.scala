...
class NiphlNumberControllerSpec extends SpecBase with MockitoSugar {
  ...
  private lazy val niphlNumberRouteCreate =
    controllers.profile.routes.NiphlNumberController.onPageLoadCreate(NormalMode).url

  private lazy val niphlNumberRouteUpdate =
    controllers.profile.routes.NiphlNumberController.onPageLoadUpdate(NormalMode).url
      ...
      "must return OK and the correct view for a GET" in {
        ...
        running(application) {
          ...
          contentAsString(result) mustEqual view(
            form,
            controllers.profile.routes.NiphlNumberController.onSubmitCreate(NormalMode)
          )(
            request,
    ...
    ".update" - {
      ...
      "must return OK and the correct view for a GET when HasNiphl hasn't been answered when there is a niphl number" in {
        ...
        running(application) {
          ...
          contentAsString(result) mustEqual view(
            form.fill("3"),
            controllers.profile.routes.NiphlNumberController.onSubmitUpdate(NormalMode)
          )(
            request,
            messages(application)
          ).toString
