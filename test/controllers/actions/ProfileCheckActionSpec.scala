...
class ProfileCheckActionSpec extends SpecBase with MockitoSugar {

  class Harness(mockTraderProfileConnector: TraderProfileConnector)
      extends ProfileCheckActionImpl(mockTraderProfileConnector) {
    def callFilter[A](request: IdentifierRequest[A]): Future[Option[Result]] = filter(request)
  }

  "Profile Check Action" - {
    "must redirect to home page when profile exists" in {
    ...
    "must not redirect when profile does not exist" in {
      val mockTraderProfileConnector = mock[TraderProfileConnector]
