  ...
  "keepAlive" - {
    ...
    "when the user has answered some questions" - {
      ...
      "must keep the answers alive and return OK" in {
        ...
        val userAnswers = UserAnswers("idWithAnswers", MaintainProfileAnswers(ukimsNumber = Some(UkimsNumber("testUkims"))))
        val application =
        ...
        running(application) {
          ...
          verify(mockSessionRepository, times(1)).keepAlive(userAnswers.id)
        }
      }
    }
