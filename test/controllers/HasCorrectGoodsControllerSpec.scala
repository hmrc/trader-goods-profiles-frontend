  ...
  "HasCorrectGoodsController2" - {
    ...
    "for the Longer Commodity Code Journey" - {
      ...
      lazy val hasCorrectGoodsRoute =
        routes.HasCorrectGoodsController.onPageLoadLongerCommodityCode2(NormalMode, testRecordId).url
      lazy val onSubmitAction: Call =
        routes.HasCorrectGoodsController.onSubmitLongerCommodityCode2(NormalMode, testRecordId)
      ...
      "for a GET" - {
        ...
        "must populate the view correctly on a GET when the question has previously been answered" in {
          ...
          val commodity   = Commodity("654321", List("Description"), Instant.now, None)
          val userAnswers = emptyUserAnswers
            .set(LongerCommodityQuery2(testRecordId), commodity)
            .success
            .value
      ...
      "for a POST" - {
        ...
        "must return a Bad Request and errors when invalid data is submitted" in {
          ...
          val userAnswers =
            emptyUserAnswers.set(LongerCommodityQuery2(testRecordId), commodity).success.value
