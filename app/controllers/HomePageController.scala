...
import models.{DownloadDataSummary, HistoricProfileData, TimeoutException}
...
class HomePageController @Inject() (
  ...
  def onPageLoad: Action[AnyContent] = (identify andThen profileAuth andThen getOrCreate).async { implicit request =>
    val result = withTimeout(goodsRecordConnector.getRecords(1, 1).flatMap { _ =>
        for {
    ...
    }, 1.second).recoverWith {
      case TimeoutException => Future.successful(
        Redirect(
