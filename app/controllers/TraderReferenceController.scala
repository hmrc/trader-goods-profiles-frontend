...
class TraderReferenceController @Inject() (
  ...
  def onPageLoad(mode: Mode, recordId: String): Action[AnyContent] = (identify andThen getData andThen requireData) {
    implicit request =>
      val preparedForm = request.userAnswers.get(TraderReferencePage(recordId)) match {
        case None        => form
    ...
    (identify andThen getData andThen requireData).async { implicit request =>
      form
        ...
        .fold(
          ...
          value =>
            for {
              updatedAnswers <- Future.fromTry(request.userAnswers.set(TraderReferencePage(recordId), value))
              _              <- sessionRepository.set(updatedAnswers)
            } yield Redirect(navigator.nextPage(TraderReferencePage(recordId), mode, updatedAnswers))
        )
    }
}
