...
class HasNirmsController @Inject() (
  ...
  def onPageLoadCreate(mode: Mode): Action[AnyContent] =
    (identify andThen checkProfile andThen getData andThen requireData) { implicit request =>
      ...
      Ok(view(preparedForm, controllers.profile.routes.HasNirmsController.onSubmitCreate(mode), mode, isCreateJourney = true))
    }
    ...
    (identify andThen checkProfile andThen getData andThen requireData).async { implicit request =>
      form
        ...
        .fold(
          formWithErrors =>
            Future.successful(
              BadRequest(view(formWithErrors, controllers.profile.routes.HasNirmsController.onSubmitCreate(mode), mode, isCreateJourney = true))
            ),
          value =>
            for {
  ...
  def onPageLoadUpdate(mode: Mode): Action[AnyContent] =
    (identify andThen profileAuth andThen getData andThen requireData).async { implicit request =>
      request.userAnswers.get(HasNirmsUpdatePage) match {
        ...
        case Some(value) =>
          Future.successful(
            Ok(view(form.fill(value), controllers.profile.routes.HasNirmsController.onSubmitUpdate(mode), mode, isCreateJourney = false))
          )
      }
    }
    ...
    (identify andThen profileAuth andThen getData andThen requireData).async { implicit request =>
      form
        ...
        .fold(
          formWithErrors =>
            Future.successful(
              BadRequest(view(formWithErrors, controllers.profile.routes.HasNirmsController.onSubmitUpdate(mode), mode, isCreateJourney = false))
            ),
          value =>
