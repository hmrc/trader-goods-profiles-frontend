...
class HasGoodDescriptionChangeController @Inject() (
  ...
  def onPageLoad(mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData) { implicit request =>
    val preparedForm = request.userAnswers.get(HasGoodDescriptionChangePage) match {
      case None        => form
    ...
    implicit request =>
      form
        ...
        .fold(
          ...
          value =>
            for {
              updatedAnswers <- Future.fromTry(request.userAnswers.set(HasGoodDescriptionChangePage, value))
              _              <- sessionRepository.set(updatedAnswers)
            } yield Redirect(navigator.nextPage(HasGoodDescriptionChangePage, mode, updatedAnswers))
        )
  }
}
